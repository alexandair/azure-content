...
-	Deep integration of VM Extensions (Custom Script, DSC, Chef, Puppet, etc.) with the Azure Resource Manager in a template file allows easy orchestration of in-VM setup configuration
...
You can find the detailed REST API documentation for the Compute, Network, and Storage Resource Providers [here](https://msdn.microsoft.com/library/azure/dn790568.aspx).
...
This is not supported at the moment. However, you can copy the VHD files from a Storage Account that was created using the Service Management APIs to a new account created using the new Azure Resource Manager APIs.
...
Yes, you can connect the Virtual Networks created using the new Azure Resource Manager APIs with an Express Route Circuit.
